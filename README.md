...
最近几轮迭代逐步把文本分析、字节格式化等能力下沉到了共享 `core` crate，并同步暴露给服务端 API 与浏览器内 WASM。本轮补了一块更偏监控与仪表盘的小工具：百分比格式化。

- 新增 `core::percent::format_percent`，统一输出 `percent` / `ratio` / `hint`；
- 新增服务端接口 `/api/percent/format`；
- WASM 导出 `format_percent`，worker 与前端对照基准已接好；
- Demo 面板新增“百分比格式化”实验，可直接比较 API 与 WASM 结果与耗时；
- 补了覆盖常见值、极小值、越界 clamp 与非法输入的单元测试。
